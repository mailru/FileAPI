...
""))},readAsImage:function(b,e,a){if(h.isFile(b))if(x){var d=x.createObjectURL(b);d===k?c(b,e,"error"):h.readAsImage(d,e,a)}else h.readAsDataURL(b,function(d){"load"==d.type?h.readAsImage(d.result,e,a):(a||"error"==d.type)&&c(b,e,d,null,{loaded:d.loaded,total:d.total})});else h.isCanvas(b)?c(b,e,"load",b):E.test(b.nodeName)?b.complete?c(b,e,"load",b):f(b,"error abort load",function B(a){"load"==a.type&&x&&x.revokeObjectURL(b.src);l(b,"error abort load",B);c(b,e,a,b)}):b.iframe?c(b,e,{type:"error"}):
(d=new Image,d.src=b.dataURL||b,h.readAsImage(d,e,a))},checkFileObj:function(b){var e={};"object"==typeof b?e=b:e.name=(b+"").split(/(\\|\/)/g).pop();e.type===k&&(e.type=e.name.split(".").pop());g(N,function(b,a){b.test(e.type)&&(e.type=a+"/"+e.type)});return e},getDropFiles:function(b,e){var a=[],c=(b.originalEvent||b||"").dataTransfer||{},d=j(c.items)&&c.items[0]&&m(c.items[0]),w=h.queue(function(){e(a)});g((d?c.items:c.files)||[],function(b){w.inc();if(d)p(b,function(b,e){!b&&a.push.apply(a,e);
w.next()});else{var e=function(e){e&&a.push(b);w.next()};if(!b.type&&0==b.size%4096&&102400>=b.size)if(u)try{var c=new u;f(c,L,function(b){b="error"!=b.type;e(b);b&&c.abort()});c.readAsDataURL(b)}catch(j){e(!1)}else e(null);else e(!0)}});w.check()},getFiles:function(b,a,c){var d=[];if(c)return h.filterFiles(h.getFiles(b),a,c),null;b.jquery&&(b.each(function(){d=d.concat(h.getFiles(this))}),b=d,d=[]);"string"==typeof a&&(a=h.getFilesFilter(a));b.originalEvent?b=A(b.originalEvent):b.srcElement&&(b=
A(b));b.dataTransfer?b=b.dataTransfer:b.target&&(b=b.target);b.files?d=b.files:!e&&G.test(b&&b.tagName)?h.trim(b.value)&&(d=[h.checkFileObj(b.value)],d[0].blob=b,d[0].iframe=!0):j(b)&&(d=b);!a&&G.test(b&&b.tagName)&&(a=h.getFilesFilter(b));return h.filter(d,function(b){return!a||a.test(b.name)})},getInfo:function(b,e){var a={},c=P.concat();h.isFile(b)?function B(){var d=c.shift();d?d.test(b.type)?d(b,function(b,c){b?e(b):(h.extend(a,c),B())}):B():e(!1,a)}():e("not_support",a)},addInfoReader:function(b,
...
b));v[e]=b});"abort"!=e.statusText&&p?(e.currentFile=y,b.prepare(y,v),this._getFormData(v,p,function(a){d||b.upload(e,b);var l=new h.XHR(h.extend({},v,{upload:function(){b.fileupload(y,l,v)},progress:function(a){m||(j=d+=c*w*(a.loaded/a.total)-j+0.5|0,p.total=a.total,p.loaded=a.loaded,b.fileprogress(a,y,l,v),b.progress({type:a.type,total:c,loaded:e.loaded=d,lengthComputable:!0},y,l,v))},complete:function(a){g(W,function(b){e[b]=l[b]});m=!0;p.loaded=p.total;e.loaded=d+=d-j+(c*w+0.5)|0;b.filecomplete(a,
l,y,v);M.call(f)}}));w=p.size/c;e.abort=function(){l.abort()};l.send(a)})):b.complete(200==e.status?!1:e.statusText||"error",e,b)}).call(this);return e},_getFilesDataArray:function(b){var e=[],a={};if(G.test(b&&b.tagName)){var c=h.getFiles(b);a[b.name||"file"]=null!==b.getAttribute("multiple")?c:c[0]}else j(b)&&G.test(b[0]&&b[0].tagName)?g(b,function(b){a[b.name||"file"]=h.getFiles(b)}):a=b;g(a,function B(b,a){j(b)?g(b,function(b,e){B(b,a+"["+e+"]")}):b&&b.name&&e.push({name:a,file:b,size:b.size,
total:b.size,loaded:0})});return e},_getFormData:function(b,e,a){var c=e.file,d=e.name,j=c.name,w=c.type;e=h.support.transform&&b.imageTransform;var p=new h.Form,f=h.queue(function(){a(p)}),v=e&&(0<parseInt(e.maxWidth||e.minWidth||e.width,10)||e.rotate);h.Image&&e&&(/image/.test(c.type)||T.test(c.nodeType))?(f.inc(),v&&(e=[e]),h.Image.transform(c,e,b.imageAutoOrientation,function(e,a){v&&!e?(!y&&!h.flashEngine&&(a[0]=h.toBinaryString(a[0]),p.multipart=!0),p.append(d,a[0],j,w)):(e||(g(a,function(b,
e){!y&&!h.flashEngine&&(b=h.toBinaryString(b),p.multipart=!0);p.append(d+"["+e+"]",b,j,w)}),d+="[original]"),(e||b.imageOriginal)&&p.append(d,c,j,w));f.next()})):p.append(d,c,j);g(b.data,function X(b,e){"object"==typeof b?g(b,function(b,a){X(b,e+"["+a+"]")}):p.append(e,b)});f.check()},reset:function(b){var e,a;t?(a=t(b).clone(!0).insertBefore(b).val("")[0],t(b).remove()):(e=b.parentNode,a=e.insertBefore(b.cloneNode(!0),b),a.value="",e.removeChild(b),g(C[h.uid(b)],function(e,c){g(e,function(e){l(b,
