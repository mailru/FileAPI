...
d[h&&h.exif&&h.exif.Orientation]||0;f._trans(c)}):f._trans(c)}else c("not_support")},toData:function(a){this.get(a)}};m.exifOrientation=d;m.transform=function(c,d,f,h){a.getInfo(c,function(n,l){var w={},q=a.queue(function(a){h(a,w)});n?q.fail():a.each(d,function(a,d){if(!q.isFail()){var k=m(l.nodeType?l:c);if("function"==typeof a)a(l,k);else if(a.width)k[a.preview?"preview":"resize"](a.width,a.height,a.type);else a.maxWidth&&(l.width>a.maxWidth||l.height>a.maxHeight)&&k.resize(a.maxWidth,a.maxHeight,
"max");a.rotate===g&&f&&(a.rotate="auto");k.rotate(a.rotate);q.inc();k.toData(function(a,c){a?q.fail():(w[d]=c,q.next())})}})})};a.support.canvas=a.support.transform=c;a.Image=m})(FileAPI,document);
(function(a,l,g){var m=l.encodeURIComponent,n=l.FormData;l=function(){this.items=[]};l.prototype={append:function(a,c,d,h){this.items.push({name:a,blob:c&&c.blob||(void 0==c?"":c),file:c&&(d||c.name),type:c&&(h||c.type)})},each:function(a){for(var c=0,d=this.items.length;c<d;c++)a.call(this,this.items[c])},toData:function(f,c){c._chunked=a.support.chunked&&0<c.chunkSize&&1==a.filter(this.items,function(a){return a.file}).length;a.support.html5?this.multipart?(a.log("FileAPI.Form.toMultipartData"),
...
a.support.transform=!0;k(a,{getFiles:function(e,c,d){if(d)return a.filterFiles(a.getFiles(e),c,d),null;var f=a.isArray(e)?e:q[a.uid(e.target||e.srcElement||e)];if(!f)return this.parent.apply(this,arguments);c&&(c=a.getFilesFilter(c),f=a.filter(f,function(a){return c.test(a.name)}));return f},getInfo:function(e,c){if(e&&!e.flashId)this.parent.apply(this,arguments);else{if(!e.__info){var d=e.__info=a.defer();p.cmd(e,"getFileInfo",{id:e.id,callback:A(function E(a,c){v(E);d.resolve(a,e.info=c)})})}e.__info.then(c)}}});
a.support.transform=!0;a.Image&&k(a.Image.prototype,{get:function(a,c){this.set({scaleMode:c||"noScale"});this.parent(a)},_load:function(c,d){a.log("FileAPI.Image._load:",c);if(c&&!c.flashId)this.parent.apply(this,arguments);else{var f=this;a.getInfo(c,function(a){d.call(f,a,c)})}},_apply:function(c,d){a.log("FileAPI.Image._apply:",c);if(c&&!c.flashId)this.parent.apply(this,arguments);else{var f=this.getMatrix(c.info);p.cmd(c,"imageTransform",{id:c.id,matrix:f,callback:A(function E(k,l){a.log("FileAPI.Image._apply.callback:",
k);v(E);if(k)d(k);else if(!a.support.dataURI||3E4<l.length){var m={width:!(f.deg%180)?f.dw:f.dh,height:f.deg%180?f.dw:f.dh,scale:f.scaleMode},n=d,r=function(){try{p.get(s).setImage(l)}catch(c){a.log('flash.setImage -- can not set "base64":',c)}},q,s=a.uid(),t=g.createElement("div");for(q in m)t.setAttribute("data-img-"+q,m[q]);x(t,m);t.innerHTML=h(a.extend({id:s,src:D(a.flashImageUrl,"r="+a.uid()),wmode:"opaque",flashvars:"scale="+m.scale+"&callback="+A(function z(){v(z);setTimeout(r,99);return!0})},
