...
a.queue(function(){n.cmd(k,"upload",{url:E(c.url),data:f,files:g,headers:c.headers,callback:A(function z(d){var f=d.type,g=d.result;a.log("flash.upload."+f+":",d);if("progress"==f)d.loaded=Math.min(d.loaded,d.total),d.lengthComputable=!0,c.progress(d);else if("complete"==f)w(z),"string"==typeof g&&(j.responseText=g.replace(/%22/g,'"').replace(/%5c/g,"\\").replace(/%26/g,"&").replace(/%25/g,"%")),j.end(d.status||200);else if("abort"==f||"error"==f)j.end(d.status||0,d.message),w(z)})})});a.each(g,function(c){m.inc();
