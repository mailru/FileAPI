...
v=!(!w||!l||!a.Uint8Array&&!x&&!d.prototype.sendAsBinary)&&!(/safari\//.test(t)&&/windows/i.test(t)),t=v&&"withCredentials"in new d,s=v&&!!s&&!(!s.prototype.webkitSlice&&!s.prototype.mozSlice&&!s.prototype.slice),N=a.document,F=a.dataURLtoBlob,T=/img/i,U=/canvas/i,V=/img|canvas/,L=/input/i,I=/^data:[^,]+,/,G=Math.pow,W=Math.round,A=Number,x=function(b){return W(b*this)},H=new A(1024),O=new A(G(H,2)),J=new A(G(H,3)),G=new A(G(H,4)),D={},P=[],K="abort progress error load loadend",X="status statusText readyState response responseXML responseText responseBody".split(" "),
...
a.event.on(g,"mouseover",l.mouseover);a.event.on(g,"click",function(a){l.mouseover(a)&&(a.preventDefault?a.preventDefault():a.returnValue=!0)})},getWrapper:function(a){do if(/js-fileapi-wrapper/.test(a.className))return a;while((a=a.parentNode)&&a!==g.body)},mouseover:function(d){d=a.event.fix(d).target;if(/input/i.test(d.nodeName)&&"file"==d.type){var c=d.getAttribute(t);if("i"==c||"r"==c)return!1;if("p"!=c){d.setAttribute(t,"i");var c=g.createElement("div"),e=l.getWrapper(d);if(!e){a.log("flash.mouseover.error: js-fileapi-wrapper not found");
return}k(c,{top:0,left:0,width:d.offsetWidth+100,height:d.offsetHeight+100,zIndex:"1000000",position:"absolute"});e.appendChild(c);l.publish(c,a.uid());d.setAttribute(t,"p")}return!0}},event:function(d){var c=d.type;if("ready"==c){try{l.getInput(d.flashId).setAttribute(t,"r")}catch(e){}l.ready();setTimeout(function(){l.mouseenter(d)},50);return!0}"ping"===c?a.log("(flash -> js).ping:",[d.status,d.savedStatus],d.error):"log"===c?a.log("(flash -> js).log:",d.target):c in l&&setTimeout(function(){a.log("Flash.event."+
