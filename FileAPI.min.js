...
d){c||(a=a.concat(d));b?b():e(!1,a)})})},function(){e("directory_reader")})}else p(m(b),e);else e("empty_entry")}function j(b){return"object"==typeof b&&b&&"length"in b}function A(b){b.target||(b.target=a.event&&a.event.srcElement||w);3===b.target.nodeType&&(b.target=event.target.parentNode);return b}var v=1,E=function(){},s=navigator.userAgent,x=a.createObjectURL&&a||a.URL&&URL.revokeObjectURL&&URL||a.webkitURL&&webkitURL,r=a.File,u=a.FileReader,n=a.FormData,t=a.jQuery,e=!(!r||!(u&&a.Uint8Array||
n))&&!(/safari\//.test(s)&&/windows/i.test(s)),s=e&&"withCredentials"in new XMLHttpRequest,n=e&&!!Blob&&!(!Blob.prototype.webkitSlice&&!Blob.prototype.mozSlice&&!Blob.prototype.slice),w=a.document,y=a.dataURLtoBlob,P={},F=/img/i,U=/canvas/i,V=/img|canvas/,H=/input/i,W=/^data:[^,]+,/,z=Math.pow,X=Math.round,I=Number,J=function(b){return X(b*this)},G=new I(1024),Q=new I(z(G,2)),K=new I(z(G,3)),z=new I(z(G,4)),C={},R=[],L="abort progress error load loadend",Y="status statusText readyState response responseXML responseText responseBody".split(" "),
h={version:"1.2.1",cors:!1,debug:!1,pingUrl:!1,staticPath:"./",flashUrl:0,flashImageUrl:0,chunkSize:0,chunkUploadRetry:0,KB:(G.from=J,G),MB:(Q.from=J,Q),GB:(K.from=J,K),TB:(z.from=J,z),expando:"fileapi"+(new Date).getTime(),uid:function(b){return b?b[h.expando]=b[h.expando]||h.uid():(++v,h.expando+v)},log:function(){h.debug&&(a.console&&console.log)&&(console.log.apply?console.log.apply(console,arguments):console.log([].join.call(arguments," ")))},getXHR:function(){var b;if(a.XMLHttpRequest)b=new XMLHttpRequest;
...
p?(e.currentFile=y,b.prepare(y,v),this._getFormData(v,p,function(a){d||b.upload(e,b);var l=new h.XHR(h.extend({},v,{upload:function(){b.fileupload(y,l,v)},progress:function(a){!m&&a.lengthComputable&&(j=d+=c*w*(a.loaded/a.total)-j+0.5|0,p.total=a.total,p.loaded=a.loaded,b.fileprogress(a,y,l,v),b.progress({type:a.type,total:c,loaded:e.loaded=d,lengthComputable:!0},y,l,v))},complete:function(a){g(Y,function(b){e[b]=l[b]});m=!0;p.loaded=p.total;e.loaded=d+=d-j+(c*w+0.5)|0;b.filecomplete(a,l,y,v);M.call(f)}}));
w=p.size/c;e.abort=function(){l.abort()};l.send(a)})):b.complete(200==e.status?!1:e.statusText||"error",e,b)}).call(this);return e},_getFilesDataArray:function(b){var e=[],a={};if(H.test(b&&b.tagName)){var c=h.getFiles(b);a[b.name||"file"]=null!==b.getAttribute("multiple")?c:c[0]}else j(b)&&H.test(b[0]&&b[0].tagName)?g(b,function(b){a[b.name||"file"]=h.getFiles(b)}):a=b;g(a,function B(b,a){j(b)?g(b,function(b,e){B(b,a+"["+e+"]")}):b&&b.name&&e.push({name:a,file:b,size:b.size,total:b.size,loaded:0})});
return e},_getFormData:function(b,e,a){var c=e.file,d=e.name,j=c.name,w=c.type;e=h.support.transform&&b.imageTransform;var p=new h.Form,f=h.queue(function(){a(p)}),v=e&&(0<parseInt(e.maxWidth||e.minWidth||e.width,10)||e.rotate);e&&(/image/.test(c.type)||V.test(c.nodeType))?(f.inc(),v&&(e=[e]),h.Image.transform(c,e,b.imageAutoOrientation,function(e,a){v&&!e?(!y&&!h.flashEngine&&(a[0]=h.toBinaryString(a[0]),p.multipart=!0),p.append(d,a[0],j,w)):(e||(g(a,function(b,e){!y&&!h.flashEngine&&(b=h.toBinaryString(b),
...
k.throttle(function(a){a=k.extend({},a,{loaded:f.start+a.loaded,totalSize:f.size,total:f.size});g.progress(a,c,g)},100),!1),d.onreadystatechange=function(){c.status=d.status;c.statusText=d.statusText;c.readyState=d.readyState;if(4==d.readyState){for(var a in{"":1,XML:1,Text:1,Body:1})c["response"+a]=d["response"+a];d.onreadystatechange=null;0<d.status-201?++f.retry<=g.chunkUploadRetry&&(500==d.status||416==d.status)?(f.end=f.start,c._send(g,f)):c.end(d.status):(f.retry=0,f.end==f.size-1?c.end(d.status):
c._send(g,f));d=null}},f.start=f.end,f.end=Math.min(f.end+g.chunkSize,f.size)-1,(m="slice")in f.file||(m="mozSlice")in f.file||(m="webkitSlice"),d.setRequestHeader("Content-Range","bytes "+f.start+"-"+f.end+"/"+f.size),d.setRequestHeader("Content-Disposition","attachment; filename="+encodeURIComponent(f.name)),m=f.file[m](f.start,f.end+1),d.send(m),m=null):(d.upload&&d.upload.addEventListener("progress",k.throttle(function(a){g.progress(a,c,g)},100),!1),d.onreadystatechange=function(){c.status=d.status;
c.statusText=d.statusText;c.readyState=d.readyState;if(4==d.readyState){for(var a in{"":1,XML:1,Text:1,Body:1})c["response"+a]=d["response"+a];d.onreadystatechange=null;c.end(d.status);d=null}},k.isArray(f)?(d.setRequestHeader("Content-Type","multipart/form-data; boundary=_"+k.expando),f=f.join("")+"--_"+k.expando+"--",d.sendAsBinary?d.sendAsBinary(f):(m=Array.prototype.map.call(f,function(a){return a.charCodeAt(0)&255}),d.send((new Uint8Array(m)).buffer))):d.send(f)))}};k.XHR=q})(window,FileAPI);
(function(a,k,g){var q=a.support,l=k.navigator,f=l.mimeTypes,c=!1;if(l.plugins&&"object"==typeof l.plugins["Shockwave Flash"])c=l.plugins["Shockwave Flash"].description&&!(f&&f["application/x-shockwave-flash"]&&!f["application/x-shockwave-flash"].enabledPlugin);else try{c=!(!k.ActiveXObject||!new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))}catch(d){}q.flash=c;if(a.support.flash&&(!a.support.html5||a.cors&&!a.support.cors)){var m=function(a){return('<object id="#id#" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+
...
e.type+":",e);n[c](e)},1)},mouseenter:function(a){var c=n.getInput(a.flashId);c&&(n.cmd(a,"multiple",null!==c.getAttribute("multiple")),n.cmd(a,"accept",(c.getAttribute("accept")||"*").replace(/\./g,"")))},get:function(a){return g[a]||k[a]||g.embeds[a]},getInput:function(a){try{var c=n.getWrapper(n.get(a));if(c)return c.getElementsByTagName("input")[0]}catch(d){}},select:function(e){var c=n.getInput(e.flashId),d=a.uid(c);e=e.target.files;a.each(e,function(e){a.checkFileObj(e)});r[d]=e;g.createEvent?
(d=g.createEvent("Event"),d.initEvent("change",!0,!1),c.dispatchEvent(d)):g.createEventObject&&(d=g.createEventObject(),c.fireEvent("onchange",d))},cmd:function(e,c,d,f){try{return a.log("(js -> flash)."+c+":",d),n.get(e.flashId||e).cmd(c,d)}catch(g){a.log("(js -> flash).onError:",g),f||setTimeout(function(){n.cmd(e,c,d,!0)},50)}},patch:function(){a.flashEngine=a.support.transform=!0;j(a,{getFiles:function(e,c,d){if(d)return a.filterFiles(a.getFiles(e),c,d),null;var f=a.isArray(e)?e:r[a.uid(e.target||
e.srcElement||e)];if(!f)return this.parent.apply(this,arguments);c&&(c=a.getFilesFilter(c),f=a.filter(f,function(a){return c.test(a.name)}));return f},getInfo:function(a,c){if(a&&!a.flashId)this.parent.apply(this,arguments);else{if(!a.__info){var d=a.__info=FileAPI.defer();n.cmd(a,"getFileInfo",{id:a.id,callback:A(function F(c,f){v(F);d.resolve(c,a.info=f)})})}a.__info.then(c)}}});a.support.transform=!0;j(FileAPI.Image.prototype,{get:function(a,c){this.set({scaleMode:c||"noScale"});this.parent(a)},
_load:function(c,d){a.log("FileAPI.Image._load:",c);if(c&&!c.flashId)this.parent.apply(this,arguments);else{var f=this;a.getInfo(c,function(a){d.call(f,a,c)})}},_apply:function(c,d){a.log("FileAPI.Image._apply:",c);if(c&&!c.flashId)this.parent.apply(this,arguments);else{var f=this.getMatrix(c.info);n.cmd(c,"imageTransform",{id:c.id,matrix:f,callback:A(function F(j,k){a.log("FileAPI.Image._apply.callback:",j);v(F);if(j)d(j);else if(!a.support.dataURI||3E4<k.length){var l={width:!(f.deg%180)?f.dw:f.dh,
height:f.deg%180?f.dw:f.dh,scale:f.scaleMode},q=d,s=function(){try{n.get(t).setImage(k)}catch(a){}},r,t=a.uid(),u=g.createElement("div");for(r in l)u.setAttribute("data-img-"+r,l[r]);p(u,l);u.innerHTML=m(a.extend({id:t,src:E(a.flashImageUrl,"r="+a.uid()),wmode:"opaque",flashvars:"scale="+l.scale+"&callback="+A(function K(){v(K);setTimeout(s,99);return!0})},l));q(!1,u);u=null}else{var x=new Image;a.event.one(x,"error abort load",function(a){d("load"!=a.type&&a.type,x);x=null});x.src="data:"+c.type+
";base64,"+k}})})}},toData:function(a){var c=this.file;c&&!c.flashId?this.parent.apply(this,arguments):a.call(this,!c.info,{id:c.id,flashId:c.flashId,name:c.name,type:c.type,matrix:this.getMatrix(c.info)})}});j(a.Form.prototype,{toData:function(c){for(var d=this.items,f=d.length;f--;)if(d[f].file&&d[f].blob&&!d[f].blob.flashId)return this.parent.apply(this,arguments);a.log("flash.Form.toData");c(d)}});j(a.XHR.prototype,{_send:function(c,d){if(d.nodeName||d.append&&a.support.html5||a.isArray(d)&&"string"===
typeof d[0])return this.parent.apply(this,arguments);var f={},g={},j=this,k,l;a.each(d,function(a){a.file?(g[a.name]=a={id:a.blob.id,name:a.blob.name,matrix:a.blob.matrix,flashId:a.blob.flashId},l=a.id,k=a.flashId):f[a.name]=a.blob});a.log("flash.XHR._send:",k,l,g);j.xhr={headers:{},abort:function(){n.cmd(k,"abort",l)},getResponseHeader:function(a){return this.headers[a]},getAllResponseHeaders:function(){return this.headers}};var m=a.queue(function(){n.cmd(k,"upload",{url:E(c.url),data:f,files:g,
headers:c.headers,callback:A(function z(d){var f=d.type,g=d.result;a.log("flash.upload."+f+":",d);if("progress"==f)d.loaded=Math.min(d.loaded,d.total),d.lengthComputable=!0,c.progress(d);else if("complete"==f)v(z),"string"==typeof g&&(j.responseText=g.replace(/%22/g,'"').replace(/%5c/g,"\\").replace(/%26/g,"&").replace(/%25/g,"%")),j.end(d.status||200);else if("abort"==f||"error"==f)j.end(0,d.message),v(z)})})});a.each(g,function(c){m.inc();a.getInfo(c,m.next)});m.check()}})}};a.Flash=n;var t=new Image;
a.event.one(t,"error load",function(){a.support.dataURI=!(1!=t.width||1!=t.height);t=null;n.init()});t.src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=="}})(FileAPI,window,document);"undefined"!==typeof ajs&&ajs.loaded&&ajs.loaded("{fileapi}FileAPI.min");"function"===typeof define&&define.amd&&define("FileAPI",[],function(){return window.FileAPI||{}});