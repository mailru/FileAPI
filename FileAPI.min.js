...
(function(a,m){function g(b,t,d){if(b)if(w(b))for(var a=0,c=b.length;a<c;a++)a in b&&t.call(d,b[a],a,b);else for(a in b)b.hasOwnProperty(a)&&t.call(d,b[a],a,b)}function s(b,t,d){if(b){var a=k.uid(b);C[a]||(C[a]={});g(t.split(/\s+/),function(t){x?x.event.add(b,t,d):(C[a][t]||(C[a][t]=[]),C[a][t].push(d),b.addEventListener?b.addEventListener(t,d,!1):b.attachEvent?b.attachEvent("on"+t,d):b["on"+t]=d)})}}function p(b,t,d){if(b){var a=k.uid(b),c=C[a]||{};g(t.split(/\s+/),function(t){if(x)x.event.remove(b,
...
b,d)})});a.FileAPI=k.extend(k,a.FileAPI);k.log("FileAPI.version: "+k.version);A.doctype?k.log("doctype: ["+A.doctype.name+"] "+A.doctype.publicId+" "+A.doctype.systemId):k.log("FileAPI.doctype: unknown");g(A.getElementsByTagName("meta"),function(b){/x-ua-compatible/i.test(b.getAttribute("http-equiv"))&&k.log("FileAPI.meta.http-equiv: "+b.getAttribute("content"))});k.flashUrl||(k.flashUrl=k.staticPath+"FileAPI.flash.swf");k.flashImageUrl||(k.flashImageUrl=k.staticPath+"FileAPI.flash.image.swf")})(window);
