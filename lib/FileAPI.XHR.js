(function (window, api){
	...
	XHR.prototype = {
		...
		_send: function (options, data){
			var _this = this, xhr, uid = _this.uid, url = options.url, xhrForm;
			...
			if( data.nodeName ){
				...
				xhrForm = '<form target="'+ uid +'" action="'+ url +'" method="POST" enctype="multipart/form-data" style="position: absolute; top: -1000px; overflow: hidden; width: 1px; height: 1px;">'
							+ '<iframe name="'+ uid +'" src="javascript:false;"></iframe>';
				if (otions.callback) {
					xhrForm += '<input value="'+ uid +'" name="callback" type="hidden"/>'
				}
				xhrForm += '</form>'
				xhr.innerHTML = xhrForm;
