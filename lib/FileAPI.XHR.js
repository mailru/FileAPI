...
(function (window, api){
	...
	XHR.prototype = {
		...
		_send: function (options, data){
			...
			if( data.nodeName ){
                if (options.addCallbackNameToUrl) {
                    if (url.indexOf('?') >= 0) {
                        url += "&callback="+encodeURIComponent(uid);
                    } else {
                        url += "?callback="+encodeURIComponent(uid);
                    }
                }
				...
				xhr.innerHTML = '<form target="'+ uid +'" action="'+ url +'" method="POST" enctype="multipart/form-data" style="position: absolute; top: -1000px; overflow: hidden; width: 1px; height: 1px;">'
							...
							+ (options.addCallbackNameToUrl ? '' : '<input value="'+ uid +'" name="callback" type="hidden"/>')
							+ '</form>'
				;
				...
				// jsonp-callack
                var func = function (status, statusText, response){
					_this.readyState	= 4;
                ...
                var timer = function () {
                    try {
                        var transport = xhr.getElementsByTagName('iframe')[0];
                        var result = transport.contentWindow.result;
                        if (result != null) {
                            func(result.status, result.statusText, result.response);
                            return;
                        }
                    } catch (e) {
                    }
                    if (repeats++ < 50) {
                        window.setTimeout(timer, 500+repeats*100);
                    }
                };
