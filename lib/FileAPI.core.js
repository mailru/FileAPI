...
(function (window, undef){
	...
	var
		...
		support = html5 && cors, // support FileAPI (FileAPI + CORS)
		...
		api = {
			...
			checkFileObj: function (name){
				...
				_each(_rmime, function (mime, type){
			...
			getDropFiles: function (evt, callback){
				var
					...
					, entrySupport = isArray(dataTransfer.items)
			...
			getFiles: function (input, filter, callback){
				...
				else if( !support && isInputFile(input) ){
				...
				(function _nextFile(){
					...
					if( proxyXHR.statusText != 'abort' && data ){
						this._getFormData(options, data, function (form){
							...
							var
								_file = data.file,
								xhr = new api.XHR(api.extend({}, options, {

									upload: function (){
									// emit "fileupload" event
									options.fileupload(_file, xhr, options);
									},

									progress: function (evt){
										if( evt.lengthComputable ){
											loaded += ~~(total * _part * (evt.loaded/evt.total) - _loaded + .5);
											_loaded = loaded;

											// emit "fileprogress" event
											options.fileprogress(evt, _file, xhr, options);
									...
									complete: function (err){
										...
										// bytes loaded
										proxyXHR.loaded = (loaded += (loaded - _loaded) + ~~(total*_part + .5));

										// emit "filecomplete" event
										options.filecomplete(err, xhr, _file, options);
						...
						options.complete(proxyXHR.status == 200 ? false : (proxyXHR.statusText || 'error'), proxyXHR, options);
					}
				}).call(this);
	...
	function _readAs(file, fn, type, encoding){
		if( FileReader ){
			...
			try {
				// ReadAs ...
				if( encoding ){
					Reader['readAs'+type](encoding, file);
				}
				else {
			...
			_emit(file, fn, 'error', undef, { error: 'filreader_not_support' });
		}
	}
	...
	// Add default mime
	_each({     // default extensions by mime
