...
(function (api, window, document){
	...
	if( 1 && (0 || !api.support.cors) && api.support.flash ) (function (){
		var
			  _attr  = api.uid()
			, _retry = 0
			, _files = {}
...
//						, src: _getUrl('http://v.demidov.boom.corp.mail.ru/uploaderfileapi/FlashFileAPI.swf?1')
		...
		function _getUrl(url){
			if( !_rhttp.test(url) ){
				if( /^\.\//.test(url) || '/' != url.charAt(0) ){
					...
					url = path +'/'+ url.substr(RegExp.$1 == './' ? 2 : 0);
				}
		...
		function _makeFlashImage(opts, base64, fn){
			...
			_css(el, opts);
			api.log('flash.image.publish');

			el.innerHTML = _makeFlashHTML({
				  id: flashId
				, src: api.staticPath + 'FileAPI.flash.image.swf?r=' + api.uid()
				, wmode: 'opaque'
				, flashvars: 'scale=noScale&callback='+_wrap(function _setData(){
					_unwrap(_setData);
